...
public class ContaController {
	...
	//MÉTODO PARA RETORNAR APENAS AS CONTAS ATIVAS
	public List<Conta> getContasAtivas(){
		return service.getContasAtivas();
	}
	...
	//É AQUI QUE IMPLEMENTA O PAGEABLE
	...
	public ContaResponse<Page<Transacao>> getExtrato(@PathVariable int idConta, Pageable pageable) {
		ContaResponse<Page<Transacao>> response = new ContaResponse<Page<Transacao>>();
		if (service.getConta(idConta) == null) {
			response.setStatus(HttpStatus.BAD_REQUEST.value());
			response.getErrors().put("1", "Não existe uma conta com esse Id");
			return response;
		...
		response.setData(service.getExtratoPage(idConta, pageable));
		response.setStatus(HttpStatus.OK.value());
		return response;
	}
